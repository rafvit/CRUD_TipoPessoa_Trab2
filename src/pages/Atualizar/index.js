...
export default function Atualizar({navigation}) {
    ...
    async function handleChangeData(){
        ...
        const {nome, cpf, telefone, dataDeNascimento, tipoPessoa} = response.data
        setNome(nome)
        setTelefone(telefone.toString())
        setTipoPessoa(tipoPessoa)
        setCpf(cpf)
        setDataDeNascimento(dataDeNascimento)
        const data = dataDeNascimento.split("T")
        setDataDeNascimento(data[0])
